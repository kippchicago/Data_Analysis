...
current.students<-filter(map.all, Season=="Spring", Year2==2014, SchoolInitials=="KAP", Grade>=2) %.%
  select(StudentID) %>% summarise(StudentID=unique(StudentID))
  
kap.map.no.classrooms<-inner_join(current.students, kap.map.1, by="StudentID")

kap.map<-left_join(kap.map.no.classrooms, class.assignments, by="StudentID")
...
kap_long<-function(.data, ID, math_temp, read_temp, labels_grade=2){
...
students<-unique(kap.map$StudentID)
pdf("graphs/KAP_long_RIT_all_students.pdf", width = 8.5, height = 11)
    lapply(students, 
           kap_long, 
           .data=kap.map, 
