...
map.kippchi.cps[School_Display_Name_Chi!="CPS"]

setnames(map.kippchi.cps, "School_Display_Name_Chi", "Legend")
...
ggplot(data=na.omit(map.kippchi.cps[Growth_Grade_Level>=5 
...
setnames(map.kipp,"School_Display_Name_Chi", "Legend")

# Create label with top performers (2 steps: 1. copy Perc_Growth and 2. paste
# TP for top performing KIPP schools)
map.kipp[Legend!="KIPP", TP_Label:=as.character(Perc_Growth)]

#KAPS Reading Top Performers are in Grades 1,2)
map.kipp[Growth_Grade_Level %in% c(1,2) 
        & Sub_Test_Name=="Reading"
        & Legend=="KIPP Ascend Primary School",
         TP_Label:=paste(TP_Label, "+", sep="")]

#KAPS Math Top Performers are in Grade 1)
map.kipp[Growth_Grade_Level==1 
         & Sub_Test_Name=="Mathematics"
         & Legend=="KIPP Ascend Primary School",
         TP_Label:=paste(TP_Label, " +", sep="")]

#KCCP Reading Top Performers are in Grade 5)
map.kipp[Growth_Grade_Level==5
         & Sub_Test_Name=="Reading"
         & Legend=="KIPP Create Middle School",
         TP_Label:=paste(TP_Label, " +", sep="")]

#KAMS Reading Top Performers are in Grades 5)
map.kipp[Growth_Grade_Level==7 
         & Sub_Test_Name=="Reading"
         & Legend=="KIPP Ascend Middle School",
         TP_Label:=paste(TP_Label, " +", sep="")]

#KAMS Math Top Performers are in Grades 8)
map.kipp[Growth_Grade_Level==8
         & Sub_Test_Name=="Mathematics"
         & Legend=="KIPP Ascend Middle School",
         TP_Label:=paste(TP_Label, " +", sep="")]
...
ggplot(data=map.kipp[Growth_Grade_Level>=5 
  ...
  scale_linetype_manual(values = c(1,  #All other KIPP
                                   2,  #KIPP National Average
                                   2,   #National Norm 
                                   1,  #KAMS
                                   #"purple",  #KAPS
                                   1)  #KCCP) 
...
ggplot(data=map.kipp[Growth_Grade_Level<5 
  ...
  scale_size_manual(values = c(2,  #All other KIPP
                               1,  #KIPP National Average
                               1,   #National Norm 
                               2,  #KAMS
                               #"purple",  #KAPS
                               1)  #KCCP) 
...
#### MAP Growth Math vs KIPP Network Primary ####
ggplot(data=map.kipp[Growth_Grade_Level<5 
                     & Sub_Test_Name=="Mathematics"]) + 
  geom_segment(aes(x=0, 
                   xend=Perc_Growth, 
