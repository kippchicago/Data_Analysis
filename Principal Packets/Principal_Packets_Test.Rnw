...
#transform dates
DailyEnrollAttend$Date<-ymd(as.character(DailyEnrollAttend$Date))
DailyEnrollAttendByGrade$Date<-ymd(as.character(DailyEnrollAttendByGrade$Date))
...
DailyEnrollAttend$WeekOfDateLabel<-paste("Week of", month(DailyEnrollAttend$WeekOfDate,label=TRUE, abbr=TRUE), day(DailyEnrollAttend$WeekOfDate), sep=" ")
...
\begin{figure*}
<<Plot_Attendance, fig.width=8, fig.height=3, out.width='\\linewidth', dev='pdf',echo=FALSE>>=

#Melt DailyEnrollAttend into long file format

ggplot(subset(DailyEnrollAttend,SchoolID!=400146), aes(x=wday(Date), y=Enrolled)) + 
  ...
  facet_grid(SchoolID~WeekInSchoolYear, scales="free_y") +
  theme_bw()
@
\end{figure*}
