...
get.map.results<-function(test.season, connection){
  # gets MAP results from Test Analsyis Database for the given season
  # Args:
  #   test.season: the season year in SeasonYY format (e.g., Fall12, Winter13,
  #                Spring10)
  #   connection: the JDBC connection established with dbConnect()
  # Values:
  #   map.scores: a data.table with the joined Assessment and Students Table
  ...
  sql.query <- paste(
    'SELECT   s.StudentFirstName,
              ...
              s.Grade,
              a.* 
     FROM   `tblAssessmentResults',test.season,'` a
     JOIN `tblStudentBySchool',test.season,'` s
     ON s.`StudentID`=a.`StudentID`
     ;', sep="")

  # Run query
  scores <- dbGetQuery(connection, sql.query)
  
  # convert from dataframe to data.table
  scores <- data.table(scores)
  
  setnames(scores, 
           c("MeasurementScale", "Grade", "TestRITScore", "TestPercentile"),
           c("Subject", 
             paste(test.season,"Grade",sep="_"),
             paste(test.season,"RIT",sep="_"), 
             paste(test.season,"Pctl",sep="_")
           )
  )
  
  quartile.name <- paste(test.season, "Quartile", sep="_")
  pctl.name <- paste(test.season,"Pctl",sep="_")
  
  scores[get(pctl.name) <25,quartile.name := 1, with=FALSE]
  scores[get(pctl.name) >=25,quartile.name := 2, with=FALSE]
  scores[get(pctl.name) >=50,quartile.name := 3, with=FALSE]
  scores[get(pctl.name) >=75,quartile.name := 4, with=FALSE]
...
map.scores<-get.map.results("Fall12", con)
...
setkey(map.scores.8, Subject,Fall12_Pctl)
...
map.scores.5 <- map.scores[Fall12_Grade==7]
             
             setkey(map.scores.5, Subject,Fall12_Pctl)
             
             
             map.scores.5[,labely:= seq(0, 100, by=(100-0)/(.N-1)),by=Subject]
             
             map.scores.5[,labelx:= min(Fall12_RIT),by=Subject]
             
             
             
             map.scores.5[,list(PctAboveGL=mean(AboveGradeLevel)),by=Subject]
             
             # create student name (RIT / Pctl) label
             map.scores.5[,student.label:=paste(StudentFirstName, StudentLastName,
                                                " (",Fall12_RIT,"/",Fall12_Pctl,")")]             
             
             map.GL.summary.5<-map.scores.5[,list(.N, NAboveGGL = sum(AboveGradeLevel),PctAboveGL=mean(AboveGradeLevel)),by=Subject]
             
             
             map.GL.summary.5[,Label:=paste("> Grade Level = \n ",round(PctAboveGL*100),"% (",NAboveGGL,"/",N,")",sep="")]
             
             map.GL.summary.5[,PctTarget:=c(.73,.65)]
             
             map.GL.summary.5[,N2Move:=round(N*PctTarget-NAboveGGL)]
             
             map.GL.summary.5[,Label2Move:=paste("# Needed for \n",round(PctTarget*100),"% Target = ",N2Move,sep="")]  
             
             
             
             map.GL.summary.5[,hl.start:=map.scores.5[map.GL.summary.5[,N-NAboveGGL],labely]]
             map.GL.summary.5[,hl.end:=map.scores.5[map.GL.summary.5[,N-NAboveGGL-N2Move],labely]]
             
             map.GL.summary.5[,hl.x:=map.scores.5[,min(labelx-69),by=Subject][,V1]]
             
             
             
kams.GL.plot<-ggplot(map.scores.5,aes(x=Fall12_RIT, y=Fall12_Pctl)) + 
               geom_hline(aes(yintercept=50), alpha=.4) +
               geom_text(aes(color=as.factor(AboveGradeLevel), x=labelx-10, 
                             y=labely, label=student.label),hjust=1, size=2.5, show_guide=FALSE) + 
               geom_point(aes(color=as.factor(AboveGradeLevel), shape=as.factor(Fall12_Quartile)), alpha=.3, size=4,show_guide=FALSE) + 
               geom_segment(aes(x=labelx-9,xend=Fall12_RIT-1,y=labely,yend=Fall12_Pctl, color=as.factor(AboveGradeLevel)), alpha=.2,show_guide=FALSE) + 
               geom_text(data=map.GL.summary.5,aes(x=240, y=16, label=Label), size=4) +
               geom_text(data=map.GL.summary.5,aes(x=240, y=10, label=Label2Move), size=4, color="#439539") +
               geom_segment(data=map.GL.summary.5,aes(x=hl.x,xend=hl.x, y=hl.start,yend=hl.end), color="#439539",size=2,lineend = "butt") +
               facet_grid(.~Subject) +
               xlim(c(80,255)) +   
               scale_shape_manual(values=c(15:18)) +  
               scale_color_manual(values=c("#E27425","#60A2D7")) +
...
# Fall 12, 6th
             ...
             map.scores[, AboveGradeLevel:=0]
             map.scores[Fall12_Pctl>=50, AboveGradeLevel:=1]
             map.scores.5 <- map.scores[Fall12_Grade==6 
                                        & (Subject=="Mathematics"
                                           | Subject=="Reading")]
             ...
             kams.GL.plot<-ggplot(map.scores.5,aes(x=Fall12_RIT, y=Fall12_Pctl)) + 
               ...
               scale_color_manual(values=c("#E27425","#60A2D7")) +
               theme_bw()+
               ggtitle("MAP KAMS: 6th Grade FALL 2012 RIT vs National Percentile by Student")             
             
             kams.GL.plot  
             
             ggsave("../../Figures/MAP_KAMS_F12_6th_Intervention.pdf",kams.GL.plot,width=10.75, height=8.25, units="in")
...
map.scores.5 <- map.scores[Fall12_Grade==5 
                           & (Subject=="Mathematics"
                              | Subject=="Reading")
...
kams.GL.plot<-ggplot(map.scores.5,aes(x=Fall12_RIT, y=Fall12_Pctl)) + 
  ...
  scale_color_manual(values=c("#E27425","#60A2D7")) +
  theme_bw()+
  ggtitle("MAP KAMS: 5th Grade FALL 2012 RIT vs National Percentile by Student")             

kams.GL.plot  

ggsave("../../Figures/MAP_KAMS_F12_5th_Intervention.pdf",kams.GL.plot,width=10.75, height=8.25, units="in")