...
plot_MAP_Results_and_Goals_2 <- function (df, plottitle=" ",labxpos=115, minx=105,alp=1) {
  ...
  p <- ggplot(df, aes(x=Fall12_RIT, y=OrderID)) +
    ...
    geom_text(data=df[GrowthCat!="Negative"],aes(x=Fall12_RIT-1, color=as.factor(Quartile), label=StudentFirstLastName), size=2, hjust=1) +
  ...
  qrtl.labels<-get_group_stats(as.data.frame(df), grp="Quartile")
  ...
  names(wqrtl.labels)[1]<-"Quartile" # realigning to the proper facet
  ...
  p <- p + geom_text(data=wqrtl.labels, aes(x=xpos, y=AvgCountID, color=factor(Quartile),label=CountLabel),vjust=0, size=3.25) +
    geom_text(data=wqrtl.labels, aes(x=xpos, y=AvgCountID, color=factor(Quartile),label=AvgLabel),vjust=1.5, size=3.25) 
