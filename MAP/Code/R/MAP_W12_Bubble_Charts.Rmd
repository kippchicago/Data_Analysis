...
map.scores.by.grade<-ddply(map.scores[,c("ID","Grade","ClassName","Subject","Fall_RIT", "Winter_RIT","TypicalFallToWinterGrowth")],
                           .(Subject, Grade, Year), function(df) orderid(df,df$Fall_RIT))
map.scores.by.class<-ddply(map.scores[,c("StudentID","Grade","ClassName","MeasurementScale","RIT_Fall", "RIT_Winter", "TypicalFallToSpringGrowth")],
                           .(MeasurementScale, ClassName), function(df) orderid(df,df$RIT_Fall))



primary<-c("K","1", "2")
map.k1<-map.scores[Grade %in% primary]
...
k1.prop<-ddply(map.k1, .(Quartile, Grade, Subject, Year), summarize, pr= sum(MetExpGrowth)/length(MetExpGrowth), NumTested=length(MetExpGrowth), NumMetGrowth=sum(MetExpGrowth))



KCCP.grades<-c("5")
map.kccp<-subset(map.scores, Grade %in% KCCP.grades)
...
kccp.prop<-ddply(map.kccp, .(Quartile, Grade, Subject, Year), summarize, pr= sum(MetExpGrowth)/length(MetExpGrowth), NumTested=length(MetExpGrowth), NumMetGrowth=sum(MetExpGrowth))

```

```{r Primary_Bubble_Chart, echo=FALSE, fig.height=10, fig.width=10}
p.hunch <- ggplot(k1.prop,aes(x=Quartile, y=pr, color =Grade), stat="identity") +
  geom_point(aes(size=NumTested, alpha=as.factor(Year))) + 
  scale_area(range = c(8,30), guide="none") + 
  geom_text(aes(x=Quartile, y=pr, label=paste(round(pr,2)*100,"%",sep=""), group=Grade),
...
```{r Primary_Bubble_Chart, echo=FALSE, fig.height=10, fig.width=10}
p.kccp <- ggplot(kccp.prop,aes(x=Quartile, y=pr, color =Grade), stat="identity") +
