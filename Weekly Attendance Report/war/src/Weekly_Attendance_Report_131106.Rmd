...
#knit2html("Weekly_Attendance_Report_131106.Rmd", output="test.html", options=c('fragment_only', 'skip_style')); system('cp ./test.html ~/Sites/test.local/'); system('cp -r ./figure/* ~/Sites/test.local/figure/')
...
**Note (12/9/2013)*** KAP did not enter IMPACT attendance on Friday, December 6th; consequently KAP attendance data is is only accurate through Thursday, December 5th.   
...
The light gray lines shows total enrollment. The green line demarcates 96% of enrollment (i.e., our regional daily attendance goal).  The black line with dots represents actualy attendence, where the dot marks the given weekdays attendance.  
...
kable(AttByStudentBySchool[School=="KAP",list(Grade, Student, ADA, Absences)], format='html', table.attr='class="table table-responsive table-hover table-condensed table-striped"', output=TRUE)
```
