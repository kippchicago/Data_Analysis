...
map_cps_read_pre <- select(map_cps, 
                           StudentID, 
                           CPSTestRIT = RD_SQRP_RIT_PRE_GRWTH) %>%
  mutate(MeasurementScale="Reading",
         StudentID=as.integer(as.character(StudentID)),
         CPSTestRIT=as.integer(as.character(CPSTestRIT)))

map_cps_math_pre <- select(map_cps, 
                           StudentID, 
                           CPSTestRIT = MT_SQRP_RIT_PRE_GRWTH) %>%
...
map_kipp$seasonMatched %>% as.data.frame %>%
  mutate(SchoolName=ifelse(SchoolInitials %in% c("KAPS", "KAMS"), 
                                                 "Ascend", 
                                                 SchoolInitials)) %>%
  group_by(SchoolName, MeasurementScale) %>%
  dplyr::summarize(N=n(), 
                   N_Typical=sum(MetTypical),
                   Pct_Typical=round(N_Typical/N*100,1)) %>%
  arrange(MeasurementScale, SchoolName) %>% ungroup %>% select(Pct_Typical)

map_kipp_2<-map_kipp$seasonMatched %>% 
  select(StudentID,
         StudentLastname, 
         StudentFirstname,
         MeasurementScale,
...
kipp_cps <- kipp_cps %>% mutate(Diff=TestRITScore - CPSTestRIT, Same=Diff==0)

filter(kipp_cps, Same!=TRUE)
