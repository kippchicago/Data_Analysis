...
school_int <- normal(0, school_int_sd, dim = 6)
school_int_eff <- rbind(0, school_int)

# random slopes
school_slope_sd_e <- greta::lognormal(0, 1)
school_slope_sd_a <- greta::lognormal(0, 1)
school_slope_e <- normal(0, school_slope_sd_e, dim = 6)
school_slope_a <- normal(0, school_slope_sd_a, dim = 6)
...
```{r bayes predictions varying slope and intercept}
posterior_school_effects <- greta::calculate(mu, draws)
...
#RMSE
sqrt(mean((posterior_summaries$mean - yca_filtered_bayes$year_end)^2))
...
fit_pool <- stan_glmer(cbind(yr_end_present, yr_end_absent) ~ (1|school_id) + cum_present + cum_absent, data = yca_filtered_bayes, family = stats::binomial("logit"),
...
pool <- matrix(pool,  # replicate to give each player the same estimates
               nrow(yca_filtered_bayes), ncol(pool), byrow = TRUE, 
               dimnames = list(yca_filtered_bayes$studentid, c("10%", "50%", "90%")))

shift_draws <- function(draws) {
  sweep(draws[, -1], MARGIN = 1, STATS = draws[, 1], FUN = "+")
}

alphas <- shift_draws(as.matrix(fit_pool))
partialpool <- summary_stats(alphas)
partialpool <- partialpool[-nrow(partialpool),]


y <- posterior_predict(fit_pool)

y[yca_filtered_bayes$yr_end_present, yca_filtered_bayes$schoolid_idx == 5]
