...
ri_cum_enroll <- as_data(yca_filtered_bayes$cum_enrolled)
ri_cum_absent <- as_data(yca_filtered_bayes$cum_absent)
ri_school_id <- as_data(yca_filtered_bayes$schoolid_idx)

ri_int <- greta::normal(0, 10)
ri_b_e <- greta::normal(0, 10)
ri_b_a <- greta::normal(0, 10)
ri_sd <- greta::cauchy(0, 3, truncation = c(0, Inf))

# random intercepts
ri_school_int_sd <- greta::lognormal(0, 1)
ri_school_int <- greta::normal(0, ri_school_int_sd, dim = 6)
ri_school_int_eff <- rbind(0, ri_school_int)


# model
ri_mu <- ri_int + ri_cum_enroll * ri_b_e + ri_cum_absent * ri_b_a + ri_school_int_eff[ri_school_id]

ri_y <- as_data(yca_filtered_bayes$year_end)

distribution(ri_y) <- greta::normal(ri_mu, ri_sd)
...
draws_ri <- greta::mcmc(ri_m, n_samples = 2000, n_cores = 4, chains = 4)

pred_test_ri <- greta::calculate(ri_mu, draws_ri)


```

```{r predicted year end random intercept model}
pred_values_ri <- colMeans(rbind(pred_test_ri$`11`, pred_test_ri$`12`, pred_test_ri$`13`, pred_test_ri$`14`))

sqrt(mean((pred_values_ri - yca_filtered_bayes$year_end)^2))

sum(yca_filtered_bayes$year_end > .975)/ length(yca_filtered_bayes$year_end)

sum(pred_values_ri > .975)/ length(pred_values_ri)
```
...
bind_cols(orig_year_end, pred_values_ri) %>% 
  ggplot() +
...
new_mu_ri <- ri_int + new_cum_enroll * ri_b_e + new_cum_absent * ri_b_a + ri_school_int_eff[new_school_id]

pred_new_data_ri <- greta::calculate(new_mu_ri, draws_ri)

new_pred_values_ri <- colMeans(rbind(pred_new_data_ri$`11`, pred_new_data_ri$`12`, pred_new_data_ri$`13`, pred_new_data_ri$`14`))

sqrt(mean((new_pred_values_ri - new_data$year_end)^2))

sum(new_data$year_end > .975)/ length(new_data$year_end)

sum(new_pred_values_ri > .975)/ length(new_pred_values_ri)
```
...
colMeans(rbind(draws$`11`, draws$`12`, draws$`13`, draws$`14`))
...
fit_pool_vary_slope <- stan_glmer(cbind(yr_end_present, yr_end_enrolled - yr_end_present) ~ cum_present + cum_absent + (1 + cum_present + cum_absent | schoolid), data = yca_filtered_bayes, family = stats::binomial("logit"),
                     prior = wi_prior,
                     prior_intercept = wi_prior, 
                     seed = SEED,
                     cores = 4,
