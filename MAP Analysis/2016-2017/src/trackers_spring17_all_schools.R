...
# Get fall
map_fall <- nwea %>%
  filter(TermName=="Fall 2016-2017",
         Grade != 0,
         MeasurementScale %in% subjs) %>%
  ...
  mutate(Current_Grade=Grade,
         Orignal_Score=TestRITScore,
         Original_Percentile=TestPercentile,
         Original_Term = "Fall 2016",
         Fall_as_Spring = TRUE,
         TermName = "Spring 2015-2016")
...
growth_df <- map_mv$growth_df %>% ungroup() %>%
  ...
  inner_join(map_mv$cdf %>% ungroup() %>% select(testid,
...
growth_df_k <- map_mv_k$growth_df %>% ungroup() %>%
  filter(match_status == "only start",
         growth_window == "Fall to Spring") %>%
  inner_join(map_mv_k$cdf %>% ungroup() %>% select(testid,
                                     current_grade:cps_imputed,
                                     tested_at_kipp),
             by=c("start_testid" = "testid")) %>%
...
growth_df_joined <- current_ps_roster %>%
  left_join(bind_rows(tibble::as_data_frame(growth_df), as_data_frame(growth_df_k)),
...
for(school in schools) {
  ...
  file_in <- sprintf("Trackers/templates/MAP Tracker - SY17 %s.xlsx", school)
  ...
  message("Saving file.")
  file_out <- sprintf("Trackers/complete/MAP Tracker - SY17 %s %s.xlsx", school, todays_date)
