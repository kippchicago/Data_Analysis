...
current.students<-filter(map.all, Season=="Spring", Year2==2014, SchoolInitials=="KCCP", Grade>=5) %.%
  select(StudentID) %>% summarise(StudentID=unique(StudentID))
  
kccp.map<-inner_join(current.students, kccp.map.1, by="StudentID")
...
kccp_long<-function(.data, ID, math_temp, read_temp, labels_grade=6){
...
kccp.map <- arrange(kccp.map, 
                   Grade, 
                   StudentLastname, 
                   StudentFirstname, 
                   MeasurementScale
                   )
students<-unique(kccp.map$StudentID)
pdf("graphs/KCCP_long_RIT_all_students.pdf", width = 8.5, height = 11)
    lapply(students, 
           kccp_long, 
           .data=kccp.map, 
