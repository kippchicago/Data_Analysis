...
out <-  bloom_winter %>%
  left_join(bloom_fall, by = c("StudentID", "Subject"))  %>%
  left_join(bloom_goals, by = c("StudentID", "Subject")) %>%
  inner_join(current_stus_bloom, by="StudentID") %>%
...
readr::write_csv(out, path="reports/KBCP_FWS_MAP_160523.csv")
