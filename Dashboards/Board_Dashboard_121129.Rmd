...
```{r get_Transfer_data, cache=TRUE}
...
transferplot.data.prep <- function (transfers.data, enrolled.data) {
  ...
  transfers.data$Month<-month(ymd_hms(transfers.data$EXITDATE), label=TRUE)
  transfers.data$Week<-week(ymd_hms(transfers.data$EXITDATE))
...
```{r prep_ANet_Data, fig.width=12, fig.height=6}

#Get data from CSV file compiled from ANet reporting website
ANet.A1A2<-read.csv('/Users/chaid/Dropbox/Consulting/KIPP Ascend/Data Analysis/ANet/Results/ANET_Results_A1A2_130124.csv')
kippcols=c("#439539", "#BCD631", "#CFCCC1")

#ANet.A1A2<-data.frame(
#  School=rep(c(rep("KAP/MS",5), rep("ANet", 5), "KCCP"),2), 
#  Grade=rep(c(rep(c(2,5:8),2),5),2),
#  Subject=c(rep("Math",11),rep("ELA",11)),
#  Percent=c(72,56,64,45,45,60,48,50,46,48,37,48,50,58,56,57,50,54,55,54,61,47)
#)

ANet.A1A2$School<-factor(ANet.A1A2$School, levels=c("KAP/MS", "KCCP", "Anet"), labels=c("KAP/MS", "KCCP", "ANet"))
ANet.A1A2$Subject<-factor(ANet.A1A2$Subject, levels=c("Math", "ELA"))
ANet.A1A2$Grade<-factor(ANet.A1A2$Grade, levels=c(2,5,6,7,8))
ANet.A1A2$Test<-factor(ANet.A1A2$Test, levels=c("A1","A2","A1A2"), labels=c("A1", "A2", "A1 & A2 Combined"))





anet.A1A2.combine.plot<-ggplot(subset(ANet.A1A2, Test=="A1 & A2 Combined"), aes(x=Grade, y=Percent)) + 
...
anet.A1A2.combined.plot 


anet.A1A2.plot<-ggplot(subset(ANet.A1A2, Test!="A1 & A2 Combined"), aes(x=Grade, y=Percent)) + 
  geom_text(aes(y=Percent+1,label=Percent, color=School), position = position_dodge(width=1), vjust=0, size=6) +
  geom_bar(aes(fill=School), position="dodge", stat='identity') + 
...
anet.A1A2.pct60.plot<-ggplot(subset(ANet.A1A2, Test!="A1 & A2 Combined"), aes(x=Grade, y=Pct60Above)) + 
  geom_text(aes(y=Pct60Above+1,label=Pct60Above, color=School), position = position_dodge(width=1), vjust=0, size=6) +
  geom_bar(aes(fill=School), position="dodge", stat='identity') + 
  facet_grid(Subject ~ Test) +
  scale_fill_manual("",values=kippcols) +
  scale_colour_manual("",values=kippcols) +
...
anet.A1A2.pct60.plot 

#Create plot showing performance above or below average

ANet.Percent.cast<-cast(ANet.A1A2.melt, Test + Grade + Subject ~ School + variable, subset=variable=="Percent")
names(ANet.Percent.cast)<-c("Test", "Grade", "Subject", "Ascend", "Create", "ANet")

ANet.Percent<-within(ANet.Percent.cast, {
  KAPMS<-(Ascend - ANet)/ANet 
  KCCP <- (Create-ANet)/ANet
  variable<-"Percent" 
       })

ANet.Sixty.cast<-cast(ANet.A1A2.melt, Test + Grade + Subject ~ School + variable, subset=variable=="Pct60Above")
...
ANet.Diffs.melt$School<-factor(ANet.Diffs.melt$School, levels=c("KAP/MS", "KCCP", "Anet"), labels=c("KAP/MS", "KCCP", "ANet"))
...
ANet.Diffs.melt$Test<-factor(ANet.Diffs.melt$Test, levels=c("A1","A2","A1A2"), labels=c("A1", "A2", "A1 & A2 Combined"))


ggplot(subset(ANet.Diffs.melt,variable=="Percent"), aes(x=Grade, y=value)) +
    geom_point(aes(color=Pos, shape=School), size=7 ) +
  facet_grid(Subject ~ Test)
  

ggplot(subset(ANet.Diffs.melt,variable=="Percent"), aes(x=as.factor(Grade), y=value)) +
    geom_bar(aes(fill=Pos, alpha=School), position="dodge", stat="identity" ) +
  facet_grid(Subject ~ Test) + 
  scale_fill_manual(values=c("#E27425","#439539")) +
  scale_alpha_discrete(range=c(.6,.8))
...
KTC.detail$Percent<-c(96,88,91,83,94,81,81,69,88,78,81,69)
