...
df.attrition.melt$Variable<-factor(df.attrition.melt$Variable, levels=c("KIPP Ascend Attrition","CPS Mobility","KIPP Ascend Mobility"))
```

```{r Mob_Attr_Plot, echo=FALSE, fig.width=8, fig.height=4}
colours <- c("#439539", "#CFCCC1", "#60A2D7")
...
```{r graph_Academic_Progress, fig.width=8, fig.height=10}
blankPanel<-rectGrob(gp=gpar(col="white"))
grid.arrange(p.isat, blankPanel, p.map.averages.slope, p.map.quartile, nrow=2)
```
...
ggplot(AttRateByWeek, aes(x=Week, y=AttRate)) + geom_bar(fill="#255694") +  coord_cartesian(ylim = c(90,100)) + geom_hline(yintercept=96, color="#E27425")
```
...
```{r plot_Transfers, fig.width=8, fig.height=7}
g.plot<-ggplot(data=subset(xferplot, Variable=="Ceiling"), aes(x=Month, y=Value)) + geom_line(aes(group=Variable), color="#E27425") + geom_bar(data=subset(xferplot, Variable!="Ceiling"), aes(x=Month, y=Value), fill="#439539", width=.5) +  facet_grid(School~., scale="free_y")
...
#Combine plot
grid.newpage()
grid.arrange(g.plot, g.tbl, heights=c(.8,.4),nrow=2)
```
